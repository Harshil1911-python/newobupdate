...
async function tsLocalApi(path, options) {
  ...
  if (parts[0] === "invoices") {
    ...
    if (method === "DELETE" && parts.length === 2) {
      const id = parseInt(parts[1], 10);
      const inv = await tsGet("invoices", id);
      if (!inv) return { ok: false, error: "Not found." };
      // remove payments linked to this invoice
      try {
        const pays = await tsGetAll("payments");
        for (const p of pays) {
          if (Number(p.invoice_id) === id) await tsDelete("payments", p.id);
        }
      } catch (e) {}
      await tsDelete("invoices", id);
      tsNotify("invoices");
      return { ok: true };
    }
...
async function tsBuildBackupPayload() {
  ...
  const productPhotos = (data.products || []).filter(p => p && (p.photo_path || p.photo || p.image)).length;
  const layoutCount = (data.invoice_layouts || []).length;
  return {
    ...
    stats: {
      ...
      invoiceLayouts: layoutCount,
      settings: Array.isArray(data.settings) ? data.settings.length : (data.settings ? 1 : 0),
    },
  };
}

/** Export selected stores as CSV text (flat rows). Photos/layouts stay as text fields when present. */
function tsStoreToCsv(storeName, rows) {
  ...
  for (const r of rows) {
    ...
    Object.keys(r).forEach(k => {
      if (k === "passwordHash" || k === "salt" || k === "password") return;
  ...
  for (const r of rows) {
    lines.push(keys.map(k => esc(r ? r[k] : "")).join(","));
  }
  return lines.join("\n");
}

async function tsExportStoreCsv(storeName) {
  const rows = await tsGetAllSafe(storeName);
  return tsStoreToCsv(storeName, rows);
}
...
async function tsRestoreBackupPayload(payload, options) {
  ...
  const onlyList = Array.isArray(options.only) ? options.only.map(String) : null;
  const onlySet = onlyList && onlyList.length ? new Set(onlyList) : null;
  ...
  // Support older backups that put arrays at top level without stores{}
  const order = [
  ...
  // Also restore any extra store keys present in backup
  const extra = Object.keys(stores).filter(k => order.indexOf(k) < 0 && k !== "format" && k !== "version");
  const names = order.concat(extra);
  let restored = 0;
  ...
  if (!restored) throw new Error("No matching data categories found in this backup for the selected filters.");
}
...
async function tsRestoreFromFile(file, options) {
  ...
  let text = "";
  try {
    if (name.endsWith(".zip") || (file.type && /zip|octet-stream/i.test(file.type))) {
      const buf = new Uint8Array(await file.arrayBuffer());
      // Some phones share JSON renamed as zip — detect raw JSON
      const head = new TextDecoder().decode(buf.slice(0, Math.min(64, buf.length))).trim();
      ...
      throw new Error("CSV/Excel restore uses the dedicated CSV/Excel buttons below, not full backup restore.");
    } else {
      text = await file.text();
    }
  } catch (e) {
    throw new Error((e && e.message) || "Could not read backup file.");
  }
  ...
  try {
    ...
    // Sometimes zip extract yields nested quotes or HTML error pages
    const i = text.indexOf("{");
    const j = text.lastIndexOf("}");
    if (i >= 0 && j > i) {
      try { payload = JSON.parse(text.slice(i, j + 1)); }
      catch (e2) { throw new Error("Could not read backup. Use an OrbitBills .zip or .json backup (not a photo)."); }
    } else {
      throw new Error("Could not read backup. Use an OrbitBills .zip or .json backup (not a photo).");
    }
  }
  if (!payload || typeof payload !== "object") throw new Error("Invalid backup structure.");
  await tsRestoreBackupPayload(payload, options);
  return { ok: true, mode: options.mode || "replace" };
}

async function tsExtractFirstZipText(buf) {
  if (!buf || buf.length < 30) throw new Error("Not a ZIP file");
  // Scan for local file header signature PK\x03\x04 (handles junk prepended by some apps)
  let offset = -1;
  for (let i = 0; i < Math.min(buf.length - 30, 4096); i++) {
    if (buf[i] === 0x50 && buf[i + 1] === 0x4b && buf[i + 2] === 0x03 && buf[i + 3] === 0x04) {
      offset = i; break;
    }
  }
  if (offset < 0) {
    // Maybe the whole file is JSON
  ...
  if (end > buf.length) throw new Error("ZIP file is truncated or corrupted.");
  ...
  if (method === 8 && typeof DecompressionStream !== "undefined") {
    try {
      ...
      try {
        ...
        throw new Error("Could not decompress ZIP. Open the zip on a computer and restore the .json inside.");
      }
    }
  }
  throw new Error("Unsupported ZIP compression. Open the zip and restore the .json file inside.");
}
